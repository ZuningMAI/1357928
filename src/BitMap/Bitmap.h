...
public:   Bitmap( Rank n = 8 ) { init( n ); } //按指定容量创建位图（为测试暂时选用较小的默认值）
   ...
   // 打印 M 的十六进制值和 ASCII 字符
   // void printM(const Bitmap& bm, Rank n) {
   //    Rank byteCount = (n + 7) / 8; // 计算字节数
   //    for (Rank i = 0; i < byteCount; i++) {
   //       printf("M[%2d] = 0x%02X ('%c')\n", 
   //                i, 
   //                bm.M[i], 
   //                isprint(bm.M[i]) ? bm.M[i] : '.'); // 不可打印字符显示为 '.'
   //    }
   // }
