...
void testVector( Rank testSize ) {
   ...
   for ( Rank i = 0; i < testSize; i++ ) //在[0,3n)中随机选择n个数
      // V.insert ( (int)dice ( (int)(i + 1) ), (T)dice ( (int)( testSize * 3 ) ) );
      V.insert(dice(int(i+1)),dice((T)testSize*3));
   PRINT ( V );
//
...
//
   printf ( "\n  ==== Test %2d. Lowpass on\n", testID++ ); PRINT ( V );
   int i = V.size(); while ( 0 < --i ) { V[i-1] *= 7; V[i-1] += V[i]; V[i-1] = V[i-1]/8;/*V[i-1] >>= 3;*/ }    PRINT ( V );
//
   printf ( "\n  ==== Test %2d. Increase\n", testID++ ); PRINT ( V );
   increase ( V ); PRINT ( V );
//
