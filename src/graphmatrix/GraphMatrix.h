...
// 顶点的基本操作：查询第v个顶点（0 <= v < n）
   virtual Tv& vertex( Rank v ) { return this->V[v].data; } //数据
   virtual Rank inDegree( Rank v ) { return this->V[v].inDegree; } //入度
   virtual Rank outDegree( Rank v ) { return this->V[v].outDegree; } //出度
   ...
   virtual VStatus& status( Rank v ) { return this->V[v].status; } //状态
   virtual Rank& dTime( Rank v ) { return this->V[v].dTime; } //时间标签dTime
   virtual Rank& fTime( Rank v ) { return this->V[v].fTime; } //时间标签fTime
   virtual Rank& parent( Rank v ) { return this->V[v].parent; } //在遍历树中的父亲
   virtual int& priority( Rank v ) { return this->V[v].priority; } //在遍历树中的优先级数
// 顶点的动态操作
...
// 边的基本操作：查询顶点v与u之间的联边（0 <= v, u < n且exists(v, u)）
   virtual EType& type( Rank v, Rank u ) { return this->E[v][u]->type; } //边(v, u)的类型
   virtual Te& edge( Rank v, Rank u ) { return this->E[v][u]->data; } //边(v, u)的数据
   virtual int& weight( Rank v, Rank u ) { return this->E[v][u]->weight; } //边(v, u)的权重
// 边的动态操作
   virtual void insert( Te const& edge, int w, Rank v, Rank u ) { //插入权重为w的边(v, u)
      if ( exists( v, u ) ) return; //确保该边尚不存在
      this->E[v][u] = new Edge<Te>( edge, w ); //创建新边
   ...
   virtual Te remove( Rank v, Rank u ) { //删除顶点v和u之间的联边（exists(v, u)）
      ...
      this->E[v][u] = NULL; //备份后删除边记录
      this->e--; this->V[v].outDegree--; this->V[u].inDegree--; //更新边计数与关联顶点的度数
