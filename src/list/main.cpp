...
void randomList ( List<T> & list, Rank n ) { //创建长度为n的列表，其元素随机取自[0, 4n)
   ListNodePosi<T> p =
      ( rand() % 2 )
         ? list.insertLast  ( dice((T) n*4))
         : list.insertFirst ( dice((T) n*4) );
   for ( Rank i = 1; i < n; i++ )
      p = rand() % 2
         ? list.insert ( dice((T) n*4), p )
         : list.insert ( p, dice((T) n*4) );
}
...
//
   ...
   Rank i = La.size(); while ( 0 < --i ) { La[i-1]->data += La[i]->data; La[i-1]->data = (La[i-1]->data)/2; /*La[i-1]->data >>= 1; */} PRINT ( La ); //call-by-rank, clear but slow
//
   printf ( "\n  ==== Test %2d. reverse\n", testID++ );
   La.reverse(); PRINT ( La );
//
   ...
   Rank r = dice( (int)(La.size() / 4) ), n = dice( (int)(La.size() * 3 / 4), (int)(La.size()) ) - r;
//
   printf( "\n  ==== Test %2d. Copy [%d, %d)\n", testID++, r, r + n );
...
//
   ...
   while ( testSize / 4 < Ld.size() ) {
      Rank N = dice( (int)(Ld.size()) ); printf ( "removing L[%d]=", N );
